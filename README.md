...
```
bash <(curl -sL https://raw.githubusercontent.com/mahyarmlk/singbox/main/singbox.sh)
```
...
![image](https://github.com/mahyarmlk/singbox/assets/39186039/a727148c-1a11-4702-80f3-ab8b46d916af)

Help message of the script:
```

Usage: singbox.sh [-t|--transport=tcp|http|grpc|ws|tuic|hysteria2|shadowtls] [-d|--domain=<domain>] [--server=<server>]
  [--regenerate] [--default] [-r|--restart] [--enable-safenet=true|false] [--port=<port>] [-c|--core=xray|sing-box]
  [--security=reality|letsencrypt|selfsigned] [-m|--menu]
  [--show-server-config] [--add-user=<username>] [--list-users] [--show-user=<username>] [--delete-user=<username>]
...
```
bash <(curl -sL https://bit.ly/realityez) --transport=grpc --domain=www.wikipedia.com --enable-safenet=true
```
